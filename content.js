async function main() {
  ...
  // Asset loading for overlay service
  try {
    let overlayHtml = await window.__GGT_assetService.loadAsset('overlay/overlay.html');
    let overlayCss = await window.__GGT_assetService.loadAsset('overlay/overlay.css');
    try {
